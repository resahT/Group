...
{    
    ...
    if($result['result'] == 'SUCCESS')
    {
        ...
        $editBookResult = $api->editBook($bookId, $title, $author, $publisher, $saletype, $publishedDate, $edition, $subjectarea, $condition, 
                         $askingprice, $description, $bUserId, $category, $uploadtime, $keywords, $image);
                
        if($editBookResult['result'] == 'SUCCESS')
        {
...
else
{
    ...
    $publishedDate  = $response['data']['pubYear'];
    $saletype       = $response['data']['saleType'];
    $subjectarea    = $response['data']['subarea'];
    $condition      = $response['data']['cond'];
    $askingprice    = $response['data']['askingPrice'];
    $category       = $response['data']['category'];
    
    list($keyword1, $keyword2)       = explode(',', $response['data']['keyword'] . ',');
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        <div id ="topContentWrap">
            ...
            <br/>
            <form id='Add Book'  class="formbox" method='POST' accept-charset='UTF-8'>

                <fieldset>

                    <!--<legend ><h2 class="capital">A<span>dd </span> <span> a </span> B<span>ook</span></h2></legend><br/>-->
					<legend><h1 style="color: #fff;">Edit Book</h1></legend>
                    <div id="dimmer">
                        <div id="dimmerText">
